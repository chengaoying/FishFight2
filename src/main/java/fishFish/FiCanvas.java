...
import cn.ohyeah.stb.key.IPTVKeyCode;
...
public class FiCanvas extends GameCanvasEngine implements Runnable {
	...
	private static final String __MODE_SWITCH_CODE = "123789";
	private int __CODE_POS = 0;
	private long __DEBUG_TIME_MILLIS;
	private boolean __DEBUG_MODE = false;
	private String __DEBUG_MESSAGE;
	private String __DEBUG_USER_MESSAGE;
	private int __DEBUG_MAX_TIME;
	private int __DEBUG_MAX_MEM;
	
	private final int Key00 = KeyCode.NUM0;
	private final int Key01 = KeyCode.NUM1;
	private final int Key02 = KeyCode.NUM2;
	private final int Key03 = KeyCode.NUM3;
	private final int Key04 = KeyCode.NUM4;
	private final int Key05 = KeyCode.NUM5;
	private final int Key06 = KeyCode.NUM6;
	private final int Key07 = KeyCode.NUM7;
	private final int Key08 = KeyCode.NUM8;
	private final int Key09 = KeyCode.NUM9;
	private final int KeyUp = KeyCode.UP;// 上键
	private final int KeyDown = KeyCode.DOWN;// 下键
	private final int KeyLeft = KeyCode.LEFT;// 左键
	private final int KeyRight = KeyCode.RIGHT;// 右键
	private final int KeyCenter = KeyCode.OK;// 中键
	private final int KeyBack = KeyCode.BACK;// back键
	

	private final byte TopLeft = Graphics.TOP | Graphics.LEFT;
	// private final byte TopRight = SGraphics.TOP|SGraphics.RIGHT;
	private final byte HcenterVcenter = Graphics.HCENTER | Graphics.VCENTER;
	private final byte BottomLeft = Graphics.BOTTOM | Graphics.LEFT;
	private final byte BottomRight = Graphics.BOTTOM | Graphics.RIGHT;
	// private final byte TopHcenter = SGraphics.TOP|SGraphics.HCENTER;
	// private final byte BottomHcenter = SGraphics.BOTTOM|SGraphics.HCENTER;
	private Purchase purchase = null;

	private MIDlet midlet;
	public int accountId = 10001;
	public String userId = "igsuper01";
	public String accountName = "igsuper01";
	public String userToken = "xiaochen";
	public int productId = 7;
	public int propId = 35; 
	public String protocolLocation = "http://localhost:8080/itvgamesh/protocol/processor";
	private AttainmentService attainmentServ;
	...
	private void calcDebugInfo() {
		...
		__DEBUG_MESSAGE = "MT: "+(__DEBUG_MAX_TIME/1000)+"ms"+", CT: "+(interval/1000)+"ms"
	...
	protected void showDebugInfo() {
		int sy = ScreenH-48;
		g.setColor(0);
		g.fillRect(0, sy, ScreenW, ScreenH);
		g.setColor(-1);
		TextView.showMultiLineText(g, __DEBUG_MESSAGE + __DEBUG_USER_MESSAGE, 1, 0, sy, ScreenW, ScreenH);
		__DEBUG_USER_MESSAGE = null;
	}
	
	public void flushGraphicsDevice() {
		if (__DEBUG_MODE) {
			calcDebugInfo();
			showDebugInfo();
			if((key & Key01) != 0){
	...
	public void addDebugUserMessage(String msg) {
		if (__DEBUG_MODE) {
			if (__DEBUG_USER_MESSAGE != null) {
				__DEBUG_USER_MESSAGE += msg;
			}
			else {
				__DEBUG_USER_MESSAGE = msg;
			}
	...
	public FiCanvas(MIDlet midlet) {
		...
		this.midlet = midlet;
		//读取参数
		String strAccountId = midlet.getAppProperty("accountId");
		String strUserId = midlet.getAppProperty("userId");
		String strProtocolLocation = midlet.getAppProperty("server");
		String strProductId = midlet.getAppProperty("productId");
		String strPropId = midlet.getAppProperty("propId");
		String strAttainmentId = midlet.getAppProperty("attainmentId");
		//String strNickName = midlet.getAppProperty("nickName");
		try {
		...
		try {
			if(strProtocolLocation != null){
				protocolLocation = strProtocolLocation;
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			if(strPropId != null){
				propId = Integer.parseInt(strPropId);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			if(strAttainmentId != null){
				attainmentId = Integer.parseInt(strAttainmentId);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (midlet.getAppProperty("accountName") != null) {
			try {
				accountName = midlet.getAppProperty("accountName");
			}
			catch (Exception e) {
				e.printStackTrace();
			}
		}
		if (midlet.getAppProperty("userToken") != null) {
			try {
		...
		System.out.println("protocolLocation:" + protocolLocation);
		System.out.println("accountId:" + accountId);
		System.out.println("productId:" + productId);

		attainmentServ = new AttainmentService(protocolLocation);
		purchase = new Purchase(this);  //购买操作所需对象
	...
	private void runPurchase() {  //订购
		...
		if ((key & KeyLeft) != 0) {// 左
			purchase.menuPos = 0;
		} else if ((key & KeyRight) != 0) {// 右
			purchase.menuPos = 1;
		} else if ((key & KeyCenter) != 0) {// 确定
	...
	/*public void keyPressed(int keyCode) {
		//System.out.println("keyCode:"+keyCode+"pressed");
		char c = 'V';
		switch (keyCode) {
		...
		// key |= Key13;
		// break;
		case IPTVKeyCode.KEY_UP:// 上键
			key |= KeyUp;
			currDirection = 2;
			keyPersistentChange(currDirection);
			if (state == 8) {// game
				clock1 = System.currentTimeMillis() + 1000;
			}
			break;
		case IPTVKeyCode.KEY_DOWN:// 下键
			// case 6://moto
			key |= KeyDown;
			currDirection = 5;
			keyPersistentChange(currDirection);
		...
		if (__MODE_SWITCH_CODE.charAt(__CODE_POS)==c) {
			if (__CODE_POS==__MODE_SWITCH_CODE.length()-1) {
				__DEBUG_MODE  = !__DEBUG_MODE;
				__CODE_POS ^= __CODE_POS;
			}
			else {
				__CODE_POS++;
			}
	...
	private void keyPersistentChange(int curDirection) {
		if (!isKeyReleased) {
			if(!keyPersistent) {
	...
	private int currDirection = 0; //主角移动方向
	private int preDirection = 0; //上一次的移动方向
	private boolean keyPersistent = false;//是否持续移动
	private boolean isKeyReleased = false; //是否支持keyReleased时间
	...
	private void runMain() {
		...
		if ((key & KeyUp) != 0) {// 上
			...
			if (mainIndex == 0) {// 继续游戏
				...
				state = 21;
		        try{
		        	rankList = attainmentServ.queryRankingList(productId, "desc", null, null, 0, 10);
					if (rankList != null) {
	...
	private void runGame() {
		if (keyState.containsMoveEventAndRemove(KeyCode.UP)) {// 上
			...
			key = 0;
		} else if ((key & Key00) != 0) {  // 菜单
			// soundStop(0);
			menuIndex = 0;
			state = 16;// menu
			key = 0;
		} else if ((key & Key07) != 0) { //无敌状态
			if (own.money >= 300) {
				own.money -= 300;
				//flag2 = 1;
				own.status = 3;
	...
	private void runMenu() {
		if ((key & KeyUp) != 0) {
		...
		} else if ((key & KeyDown) != 0) {
			// menuIndex = (menuIndex == 3?3:menuIndex + 1);
			menuIndex = (menuIndex + 1) % 3;
		} else if ((key & KeyCenter) != 0) {
			if (menuIndex == 0) {// 继续游戏
				imgMenu = null;
				System.gc();
				// if(soundState == true){
			 ...
			 */else if (menuIndex == 1) {// 帮助
				...
				try{
					GameAttainment gamerecord = attainmentServ.read(accountId, productId, attainmentId, "desc", null, null);
					if(isPaidToPlay && gamerecord == null) {
						saveRecord();
					} else if(isPaidToPlay && own.scores >= gamerecord.getScores()){
						saveRecord();
					}
				} catch (Exception e){
					System.out.println("服务器连接失败!");
	...
	private void drawRanklist(SGraphics g)throws Exception{
		...
		if (page == 0) {
			for (int i = 0; i < pageSize; i++) {
				...
				if(rankList[i].getAccountId() == accountId) {
		...
		if (page == 1) {
			if (rankList.length > 6) {
				for (int j = 6; j < rankList.length; j++) {
					...
					if(rankList[j].getAccountId() == accountId) {
	...
	private void saveRecord() {
		//recordservice = new RecordService(protocolLocation);

		try {
			// RecordStore rs = RecordStore.openRecordStore("fish", true);
			byte record[];

			ByteArrayOutputStream bout = new ByteArrayOutputStream();
			DataOutputStream dout = new DataOutputStream(bout);
			dout.writeByte(tollGate);
			...
			System.out.println("tollGate: " + tollGate);
			...
			GameAttainment gamerecord = new GameAttainment();
			//gamerecord = new GameAttainment();
			gamerecord.setData(record);
			gamerecord.setAttainmentId(attainmentId);
			gamerecord.setScores(own.scores);
			gamerecord.setRemark("remark 测试");

			// gamerecord.setScores(300);
			System.out.println("own.scores: "+own.scores);
			attainmentServ.save(accountId, userId, productId, gamerecord);
			flag = attainmentServ.getResult();
			System.out.println("flag"+ flag);
			// rs.closeRecordStore();
			dout.close();
			bout.close();
	...
	private void loadRecord() {
		...
		try {
			...
			tollGate2 = din.readByte();
			experience = din.readInt();
			lifeNum = din.readInt();
			nonceLife = din.readInt();
			money = din.readInt();
			id = din.readInt();
			scores = gamerecord.getScores();

			System.out.println("tollGate: " + tollGate2);
			System.out.println("experience: " + experience);
			System.out.println("lifeNum: " + lifeNum);
			System.out.println("nonceLife: " + nonceLife);
			System.out.println("money: " + money);
			System.out.println("id:"+ id);

			System.out.println("read record");
			System.out.println("gamerecord.getRanking: "+gamerecord.getRanking());
			// System.out.println("score: " +"\n" + own.experience);
			// System.out.println("score: " + gamerecord.getRemark());
			// rs.closeRecordStore();
			din.close();
			bin.close();
			// rs = null;
